...
class VLLMKubernetesManager:
    ...
    def build_pod_spec(self, vllm_config: Dict[str, Any], image: Optional[str] = None) -> client.V1Pod:
        ...
        env_vars.append(
            client.V1EnvVar(name="VLLM_MODEL", value=vllm_config.get("model_source", vllm_config.get("model")))
        )
        ...
        if vllm_config.get("use_cpu", False):
            env_vars.append(
                client.V1EnvVar(name="VLLM_CPU_KVCACHE_SPACE", value=str(vllm_config.get("cpu_kvcache_space", 4)))
            )
            env_vars.append(
                client.V1EnvVar(name="VLLM_CPU_OMP_THREADS_BIND", value=vllm_config.get("cpu_omp_threads_bind", "auto"))
            )
        ...
        if not vllm_config.get("use_cpu", False):
            ...
            env_vars.append(
                client.V1EnvVar(name="VLLM_TENSOR_PARALLEL_SIZE", value=str(vllm_config.get("tensor_parallel_size", 1)))
            )
            env_vars.append(
                client.V1EnvVar(
                    name="VLLM_GPU_MEMORY_UTILIZATION", value=str(vllm_config.get("gpu_memory_utilization", 0.9))
                )
            )
        ...
        volume_mounts.append(client.V1VolumeMount(name="hf-cache", mount_path="/tmp/hf_cache"))
        ...
        if self.use_persistent_cache:
            ...
            volumes.append(
                client.V1Volume(
                    name="hf-cache",
                    persistent_volume_claim=client.V1PersistentVolumeClaimVolumeSource(claim_name=self.model_cache_pvc),
                )
            )
            logger.info(f"Using PVC {self.model_cache_pvc} for model cache")
        else:
            # Use emptyDir (ephemeral - models re-downloaded each time)
            volumes.append(client.V1Volume(name="hf-cache", empty_dir=client.V1EmptyDirVolumeSource()))
        ...
        volume_mounts.append(client.V1VolumeMount(name="root-cache", mount_path="/.cache"))
        volumes.append(client.V1Volume(name="root-cache", empty_dir=client.V1EmptyDirVolumeSource()))
        ...
        if vllm_config.get("use_cpu", False):
            # CPU mode
            resource_requests = {"memory": "8Gi", "cpu": "2"}
            resource_limits = {"memory": "32Gi", "cpu": "8"}
        else:
            # GPU mode
        ...
        # Pod metadata
        metadata = client.V1ObjectMeta(name=self.POD_NAME, labels={"app": "vllm", "managed-by": "vllm-playground"})
    ...
    async def start_container(
        ...
        try:
            ...
            created_pod = await loop.run_in_executor(
                None, lambda: api.create_namespaced_pod(namespace=self.namespace, body=pod)
            )
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if not status.get("running", False):
                    ...
                    return {"ready": False, "error": "pod_stopped", "elapsed_time": round(elapsed, 1)}
        ...
        try:
            ...
            try:
                await loop.run_in_executor(
                    None, lambda: api.read_namespaced_pod(name=self.POD_NAME, namespace=self.namespace)
                )
            except ApiException as e:
                if e.status == 404:
            ...
            await loop.run_in_executor(
                ...
                lambda: api.delete_namespaced_pod(
                    name=self.POD_NAME, namespace=self.namespace, body=client.V1DeleteOptions()
                ),
            )
        ...
        try:
            ...
            log_stream = await loop.run_in_executor(
                ...
                lambda: api.read_namespaced_pod_log(
                    name=self.POD_NAME, namespace=self.namespace, follow=True, _preload_content=False
                ),
            )
            ...
            while True:
                try:
                    ...
                    line = await loop.run_in_executor(None, lambda: log_stream.readline())
    ...
    async def _ensure_service(self, port: int = 8000):
        ...
        service = client.V1Service(
            api_version="v1", kind="Service", metadata=client.V1ObjectMeta(name=self.SERVICE_NAME), spec=service_spec
        )

        try:
            # Try to create service
            await loop.run_in_executor(
                None, lambda: api.create_namespaced_service(namespace=self.namespace, body=service)
            )
            ...
            if e.status == 409:  # Already exists
                ...
                try:
                    ...
                    try:
                        await loop.run_in_executor(
                            None,
                            lambda: api.delete_namespaced_service(name=self.SERVICE_NAME, namespace=self.namespace),
                        )
